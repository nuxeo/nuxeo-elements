  ...
  class OperationButton extends mixinBehaviors([NotifyBehavior, I18nBehavior], Nuxeo.Element) {
    ...
    _filenameFromContentDisposition(contentDisposition) {
      ...
      if (extended) {
        ...
        // An undecodable `filename*` still leaves the plain form usable, so only take the raw
        // value when the server sent nothing else.
        if (!quoted && !plain) {
          return value;
        }
      }
     ...
     * percent-encoding: both decoders throw a `URIError` on such values, which would otherwise
     * turn a cosmetic naming problem into a failed download.
     */
