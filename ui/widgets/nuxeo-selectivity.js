...
.selectivity-caret:before {
  content: "\\25bc";
        ...
        templates: {
          resultItem: (opts) => (
            `<div class="selectivity-result-item${opts.disabled ? ' disabled' : ''}"
                  ...
                  data-item-id="${escape(opts.id)}">${this.resultFormatter(opts.item)}</div>`
          ),

          resultLabel: (opts) => (
            `<div class="preserve-white-space selectivity-result-label"
                  style="padding-left: ${7 + (10 * opts.depth)}px">${escape(opts.text)}</div>`
          ),

          singleSelectedItem: (opts) => (
            `${'<span class="preserve-white-space selectivity-single-selected-item" ' +
            'data-item-id="'}${escape(opts.id)}">${
    ...
    _selectionFormatter(item) {
      return escape(item.displayLabel || item.title || item.text || item);
    }
