...
function buildTableData(size) {
  const entries = [];
  for (let index = 0; index < size; index++) {
    entries.push(docs[Math.floor(Math.random() * docs.length)]);
  }
  return entries;
}

async function tableFixture(canSelectAll = false) {
  const table = (
    await fixture(
      html`
        <div>
          ...
          <nuxeo-data-table
            ...
            selection-enabled
            multi-selection
            ?select-all-enabled=${canSelectAll}
          >
            <nuxeo-data-table-column name="Title" field="dc:title" flex="100" filter-by="dc:title" sort-by="dc:title">
              <template>
...
async function setupServer(numberPages, pageSize) {
  ...
  for (let page = 0; page < numberPages; page++) {
    ...
    tableResp.numberOfPages = numberPages;
    tableResp.pageCount = numberPages;
    tableResp.pageIndex = page * pageSize;
    tableResp.pageSize = pageSize;
    tableResp.resultsCount = numberPages * pageSize;
    server.respondWith(
      'GET',
...
suite('nuxeo-data-table', () => {
  ...
  suite('select all', () => {
    ...
    test('select all disabled', async () => {
      // setup a table with select all disabled
      table = await tableFixture();
      table.items = docs;
      await flush();
